# SigGear Products

This section is the authoritative public source for SigGear product specifications.

> **Have a specific torque, speed, backlash, size or quantity requirement?** [Open an engineering request](https://github.com/SigGearDrive/SigGear-product-docs/issues/new/choose) or email [wangwanrong@siggear.com](mailto:wangwanrong@siggear.com). Our engineers will reply within 24 hours.

## Start by Application

| Application | Recommended starting products | Application guide |
| --- | --- | --- |
| Humanoid robot joints | [Robot joint actuators](robot-joint-actuators/index.md), [cycloidal joint modules](cycloidal-joint-modules/index.md) | [Humanoid robot joint selection](../applications/humanoid-robot-joint-actuators.md) |
| Quadruped robot joints | [SG-6010C](robot-joint-actuators/sg6010c.md), [SG-8021](robot-joint-actuators/sg8021.md), [CPM-80-25](cycloidal-joint-modules/cpm80-25.md), [CPM-100-25](cycloidal-joint-modules/cpm100-25.md) | [Quadruped joint selection](../applications/quadruped-robot-joint-gearboxes.md) |
| Robotic arms and cobots | [Robot joint actuators](robot-joint-actuators/index.md), [8–42 mm planetary gearboxes](planetary-gearboxes/8-42mm-planetary-gear-reducer.md) | [Robotic arm joint selection](../applications/robotic-arm-joint-actuators.md) |
| AGV and AMR wheel drives | [Hub gear motors](hub-gear-motors/index.md), [planetary gearboxes](planetary-gearboxes/index.md) | [AGV and AMR drive selection](../applications/agv-amr-wheel-drive-gearboxes.md) |
| Robot grippers and dexterous hands | [Micro gear motors](micro-gear-motors/index.md), [8–42 mm planetary gearboxes](planetary-gearboxes/8-42mm-planetary-gear-reducer.md) | [Gripper drive selection](../applications/robot-gripper-gear-motors.md) |
| Medical and laboratory equipment | [Micro gear motors](micro-gear-motors/index.md), [planetary gearboxes](planetary-gearboxes/index.md) | [Medical and laboratory motion selection](../applications/medical-device-micro-gear-motors.md) |
| Micro robotics and compact mechanisms | [Micro gear motors](micro-gear-motors/index.md), [flat BLDC motors](flat-bldc-motors/index.md), [8–42 mm planetary gearboxes](planetary-gearboxes/8-42mm-planetary-gear-reducer.md) | [Micro robotics drive selection](../applications/micro-robotics-gear-motors.md) |

## Robot Joint Actuators

Integrated planetary robot joint actuators for humanoid robots, quadruped robots, robotic arms, exoskeletons and automation systems.

[Browse robot joint actuators](robot-joint-actuators/index.md)

## Cycloidal Joint Modules

Compact high-torque cycloidal joint modules for robot joints and precision motion systems.

[Browse cycloidal joint modules](cycloidal-joint-modules/index.md)

## Planetary Gearboxes

Miniature and compact planetary gearboxes for robotics, medical devices, laboratory automation, precision instruments and industrial mechanisms.

Published model pages currently include:

- [8–42 mm planetary gearbox series](planetary-gearboxes/8-42mm-planetary-gear-reducer.md)
- 8P, 10P, 12P, 14P, 16P, 20P, 22P and 24P model pages

[Browse planetary gearboxes](planetary-gearboxes/index.md)

## Micro Gear Motors

Miniature planetary gear motors and custom motor-transmission integration for robotics, medical devices, precision instruments and compact automation.

[Browse micro gear motors](micro-gear-motors/index.md)

## Flat BLDC Motors and Joint Drives

Flat motor and integrated joint-drive solutions, including the planetary-core SigGear-Horse family and cycloidal-core SigGear-Tiger family.

[Browse flat BLDC motors and joint drives](flat-bldc-motors/index.md)

## Hub Gear Motors

Compact geared wheel-drive products for mobile robotics and automation.

[Browse hub gear motors](hub-gear-motors/index.md)

## Selection and Application Resources

- [Browse all published selection guides](../selection-guides/index.md)
- [Robot joint actuator selection guide](../selection-guides/robot-joint-actuator-selection-guide.md)
- [Industrial automation servo planetary gearbox guide](../applications/industrial-automation-servo-planetary-gearboxes.md)
- [Robot gripper and dexterous hand gear motor guide](../applications/robot-gripper-gear-motors.md)
- [Micro robotics gear motor guide](../applications/micro-robotics-gear-motors.md)
- [Browse all application guides](../applications/index.md)

## Documentation Policy

Every model or series has one canonical product page. Unverified values are not published as confirmed specifications. CAD files, detailed drawings, noise-test information and service-life data are supplied after inquiry and application review.